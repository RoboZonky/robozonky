...
public abstract class Expiring<T> implements Supplier<Optional<T>> {
    ...
    private final AtomicReference<Tuple2<T, Instant>> value = new AtomicReference<>();
    ...
    public Optional<T> get() {
        final Tuple2<T, Instant> result = value.updateAndGet(old -> {
            if (old == null || !old._2().plus(expireAfter).isAfter(Instant.now())) {
                LOGGER.trace("Retrieving new value.");
                return retrieve().map(v -> {
                    LOGGER.debug("Retrieved new value: {}.", v);
                    runWhenUpdated.run();
                    return Tuple.of(v, Instant.now());
                }).orElse(old);
            } else {
        ...
        return result == null ? Optional.empty() : Optional.ofNullable(result._1());
    }
