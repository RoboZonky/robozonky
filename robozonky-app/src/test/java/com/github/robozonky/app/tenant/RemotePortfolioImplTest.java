...
class RemotePortfolioImplTest extends AbstractZonkyLeveragingTest {
    ...
    void chargesAffectAmounts() {
        ...
        assertSoftly(softly -> {
            ...
            softly.assertThat(p.getOverview().getInvested()).isEqualTo(Money.from(15));
        });
        p.simulateCharge(1, Rating.D, Money.from(15));
        assertSoftly(softly -> {
            softly.assertThat(p.getTotal())
                    .containsOnly(Map.entry(Rating.C, Money.from(10)), Map.entry(Rating.D, Money.from(20)));
            softly.assertThat(p.getOverview().getInvested()).isEqualTo(Money.from(30));
        });
        ...
        assertSoftly(softly -> {
            ...
            softly.assertThat(p.getOverview().getInvested()).isEqualTo(Money.from(31));
        });
        ...
        assertSoftly(softly -> {
            ...
            softly.assertThat(p.getOverview().getInvested()).isEqualTo(Money.from(32));
        });
    }
}
