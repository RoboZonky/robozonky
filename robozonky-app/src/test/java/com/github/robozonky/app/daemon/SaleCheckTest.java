...
class SaleCheckTest extends AbstractZonkyLeveragingTest {
    ...
    class SomethingIsOffered {
        ...
        private final Investment soldEarlier = Investment.fresh(MockLoanBuilder.fresh(), 200)
                .setOnSmp(false)
                .setStatus(InvestmentStatus.ACTIVE)
                .build();
        private final Investment soldNow = Investment.fresh(MockLoanBuilder.fresh(), 200)
                .setOnSmp(false)
                .setStatus(InvestmentStatus.SOLD)
                .build();
        private final Investment onSmp = Investment.fresh(MockLoanBuilder.fresh(), 200)
                .setStatus(InvestmentStatus.ACTIVE)
                .setOnSmp(true)
                .build();
