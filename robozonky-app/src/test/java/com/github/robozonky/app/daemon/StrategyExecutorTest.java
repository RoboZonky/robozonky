...
class StrategyExecutorTest extends AbstractZonkyLeveragingTest {
    ...
    void purchasingNoStrategy() {
        ...
        final PurchasingOperationDescriptor d = spy(new PurchasingOperationDescriptor(tenant));
    ...
    void investingNoStrategy() {
        ...
        final StrategyExecutor<LoanDescriptor, InvestmentStrategy> exec = new StrategyExecutor<>(auth, d);
        assertThat(exec.get()).isEmpty();
        // check events
        final List<Event> events = getEventsRequested();
        assertThat(events).isEmpty();
    ...
    void investingNoItems() {
        final Zonky z = AbstractZonkyLeveragingTest.harmlessZonky(1000);
        final PowerTenant auth = mockTenant(z);
        when(auth.getInvestmentStrategy()).thenReturn(Optional.of(ALL_ACCEPTING_INVESTMENT_STRATEGY));
    ...
    void investingNoneAccepted() {
        ...
        final PowerTenant auth = mockTenant(z);
        when(auth.getInvestmentStrategy()).thenReturn(Optional.of(NONE_ACCEPTING_INVESTMENT_STRATEGY));
        final Investor builder = Investor.build(auth);
    ...
    void investingRejected() {
        ...
        doReturn(Collections.singletonList(ld)).when(d).readMarketplace(any());
        final StrategyExecutor<LoanDescriptor, InvestmentStrategy> exec2 = new StrategyExecutor<>(auth, d);
