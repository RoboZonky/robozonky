...
final class QuotaMonitor implements Runnable {
    ...
    public void run() {
        ...
        Arrays.stream(THRESHOLDS_IN_PERCENT).forEach(thresholdInPercent -> {
            ...
            if (requestsInLast600Seconds < actualThreshold) { // OK
                ...
                if (!thresholdReachedBefore) { // newly over threshold, inform
                    ...
                    if (thresholdInPercent > 90) {
                        LOGGER.warn("Request quota estimated over {} % ({}), throttling imminent.", thresholdInPercent,
                                    requestsInLast600Seconds);
                    } else {
                        LOGGER.info("Request quota estimated over {} % ({}).", thresholdInPercent, requestsInLast600Seconds);
                    }
                }
            }
        });
