...
class Activity {
    ...
    private final AtomicReference<Runnable> settler = new AtomicReference<>(
            Activity.DO_NOTHING);
    ...
    static OffsetDateTime getLatestMarketplaceAction() {
        return Activity.STATE.getValue(
                Activity.LAST_MARKETPLACE_CHECK_STATE_ID).map(s -> {
            try {
                return OffsetDateTime.parse(s);
            } catch (final DateTimeParseException ex) {
                Activity.LOGGER.debug("Failed read marketplace timestamp.",
                                      ex);
    ...
    private void persist(final boolean hasUnactionableLoans) {
        ...
        final OffsetDateTime result = hasUnactionableLoans ?
                OffsetDateTime.now().minus(Duration.from(
                        ResultTracker.CAPTCHA_DELAY).plus(Duration.ofSeconds(30)))
                : OffsetDateTime.now();
        Activity.STATE.newBatch().set(
                Activity.LAST_MARKETPLACE_CHECK_STATE_ID, result.toString()).call();
