...
remainingInterestCondition returns [MarketplaceFilterCondition result]:
    'zbývající úrok ' (
        (c1 = remainingInterestConditionRangeOpen { $result = $c1.result; })
        | (c2 = remainingInterestConditionRangeClosedLeft { $result = $c2.result; })
        | (c3 = remainingInterestConditionRangeClosedRight { $result = $c3.result; })
    ) KC
;

remainingInterestConditionRangeOpen returns [MarketplaceFilterCondition result]:
    IS min=intExpr UP_TO max=intExpr
    { $result = RemainingInterestCondition.exact($min.result, $max.result); }
;

remainingInterestConditionRangeClosedLeft returns [MarketplaceFilterCondition result]:
    MORE_THAN min=intExpr
    { $result = RemainingInterestCondition.moreThan($min.result); }
;

remainingInterestConditionRangeClosedRight returns [MarketplaceFilterCondition result]:
    LESS_THAN max=intExpr
    { $result = RemainingInterestCondition.lessThan($max.result); }
;

relativeRemainingInterestCondition returns [MarketplaceFilterCondition result]:
    'zbývající úrok ' (
        (c1 = relativeRemainingInterestConditionRangeOpen { $result = $c1.result; })
        | (c2 = relativeRemainingInterestConditionRangeClosedLeft { $result = $c2.result; })
        | (c3 = relativeRemainingInterestConditionRangeClosedRight { $result = $c3.result; })
    ) ' % původního'
;

relativeRemainingInterestConditionRangeOpen returns [MarketplaceFilterCondition result]:
    IS min=intExpr UP_TO max=intExpr
    { $result = RelativeRemainingInterestCondition.exact(Ratio.fromPercentage($min.result), Ratio.fromPercentage($max.result)); }
;

relativeRemainingInterestConditionRangeClosedLeft returns [MarketplaceFilterCondition result]:
    MORE_THAN min=intExpr
    { $result = RelativeRemainingInterestCondition.moreThan(Ratio.fromPercentage($min.result)); }
;

relativeRemainingInterestConditionRangeClosedRight returns [MarketplaceFilterCondition result]:
    LESS_THAN max=intExpr
    { $result = RelativeRemainingInterestCondition.lessThan(Ratio.fromPercentage($max.result)); }
;
