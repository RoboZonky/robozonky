...
abstract class AbstractNaturalLanguageInvestmentStrategy {

    private static final Comparator<LoanDescriptor> COMPARATOR = new PrimaryMarketplaceComparator();
    private final ParsedStrategy strategy;

    public AbstractNaturalLanguageInvestmentStrategy(final ParsedStrategy p) {
        this.strategy = p;
    }

    protected ParsedStrategy getStrategy() {
        return strategy;
    }

    abstract protected boolean needsConfirmation(final LoanDescriptor loanDescriptor);
    ...
    protected Stream<RecommendedLoan> recommend(final Collection<LoanDescriptor> loans,
        ...
        return Util.rankRatingsByDemand(strategy, splitByRating.keySet(), portfolio)
                ...
                .flatMap(d -> { // recommend amount to invest per strategy
                    final int recommendedAmount = recommendAmount(d.item(), balance, restrictions);
