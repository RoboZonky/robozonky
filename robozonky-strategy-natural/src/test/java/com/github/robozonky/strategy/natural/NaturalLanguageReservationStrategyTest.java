...
class NaturalLanguageReservationStrategyTest {
    ...
    void givenMode() {
        final DefaultValues v = new DefaultValues(DefaultPortfolio.PROGRESSIVE);
        v.setReservationMode(ReservationMode.FULL_OWNERSHIP);
        final ParsedStrategy p = new ParsedStrategy(v);
        final ReservationStrategy s = new NaturalLanguageReservationStrategy(p);
        assertThat(s.getMode()).isEqualTo(ReservationMode.FULL_OWNERSHIP);
    }
    ...
    void recommendationIsMade() {
        ...
        final ReservationStrategy s = new NaturalLanguageReservationStrategy(p);
        final PortfolioOverview portfolio = mock(PortfolioOverview.class);
        when(portfolio.getCzkAvailable()).thenReturn(BigDecimal.valueOf(1000));
        when(portfolio.getCzkInvested()).thenReturn(BigDecimal.valueOf(p.getMaximumInvestmentSizeInCzk() - 1));
        when(portfolio.getShareOnInvestment(any())).thenReturn(BigDecimal.ZERO);
